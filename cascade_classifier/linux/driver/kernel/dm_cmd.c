...
static ssize_t dm_cmd_read(struct file *f, char __user *buf, size_t len,
                           loff_t *off);
static ssize_t dm_cmd_write(struct file *f, const char __user *buf,
...
//*********************GLOBAL VARIABLES*************************************
static struct file_operations dm_cmd_fops = {.owner = THIS_MODULE,
                                             .open = dm_cmd_open,
...
static int dm_cmd_probe(struct platform_device *pdev) {
  ...
  img_mem = dma_alloc_coherent(NULL, IMG_BUFF_SIZE, &img_dma_handle,
                               GFP_DMA | __GFP_NOFAIL);
  if (!img_mem) {
  ...
  res_mem = dma_alloc_coherent(NULL, RES_BUFF_SIZE, &res_dma_handle,
                               GFP_DMA | __GFP_NOFAIL);
  if (!res_mem) {
  ...
  if (!request_mem_region(tp->mem_start, tp->mem_end - tp->mem_start + 1,
                          DRIVER_NAME)) {
    ...
    printk(KERN_INFO
           "dm_cmd_init: Successfully allocated memory region for dm_cmd\n");
  }
  /*
...
int mmap_kmem(struct file *filp, struct vm_area_struct *vma) {
  ...
  if (vma->vm_pgoff == 0) {
    ret = dma_mmap_coherent(NULL, vma, img_mem, img_dma_handle, length);
  } else
  /* #else */
  {
...
static int dm_cmd_open(struct inode *i, struct file *f) { return 0; }
static int dm_cmd_close(struct inode *i, struct file *f) { return 0; }
static ssize_t dm_cmd_read(struct file *f, char __user *buf, size_t len,
                           loff_t *off) {
  int ret;
  char tmp_arr[200];
...
static int __init dm_cmd_init(void) {
  ...
  printk(KERN_INFO "DM_CMD Created and initialized succesfully.\n");
...
static void __exit dm_cmd_exit(void) {
  ...
  printk(KERN_INFO "dm_cmd_exit: Removed Device Module \"%s\".\n", DEVICE_NAME);
}
